combine_tax_otu <- function(tax_file, otu_file, fasta_file) {
  ...
  if (!file.exists(tax_file)) stop("Tax file not found.")
  Tax <- suppressWarnings({
    df <- read.table(tax_file, header = FALSE, sep = "\t", quote = "", comment.char = "")
    if (ncol(df) >= 4) df <- df[, -4]
    names(df) <- c("UnitID", "Taxonomy", "PID")[1:ncol(df)]
  ...
  # Step 2: Process the abundance table
  if (!file.exists(otu_file)) stop("OTU/ASV table not found.")
  ...
  # Step 4: Detect OTU or ASV based on prefix
  prefix <- substr(Tax$UnitID[1], 1, 3)
  mode <- ifelse(toupper(prefix) == "ASV", "ASV", "OTU")
  message("Detected mode: ", mode)
  
  # Step 5: Merge Tax + OTU/ASV tables
  if (!"UnitID" %in% colnames(Tax) || !"UnitID" %in% colnames(OTU))
    stop("Missing 'UnitID' column in one of the tables.")
  
  combined_data <- merge(Tax, OTU, by = "UnitID", all = FALSE)
  
  # Step 6: Return results as named list
  return(list(
    mode = mode,
...
parsing_taxa <- function(tax_otu_df, raw_otus_df, output_xlsx = "tax_assignments__to_validate.xlsx") {
  ...
  # Detect mode (OTU or ASV) from first ID available
  first_id <- if (!is.null(tax_otu_df[[1]]) && length(tax_otu_df[[1]]) > 0 && !is.na(tax_otu_df[[1]][1])) {
    as.character(tax_otu_df[[1]][1])
  } else if (!is.null(raw_otus_df[[1]]) && length(raw_otus_df[[1]]) > 0 && !is.na(raw_otus_df[[1]][1])) {
  ...
  # Merge taxonomic table with OTU sequences
  raw_data_with_seqs <- merge(
    tax_otu_df,
    raw_otus_df,
    by = ifelse(mode == "ASV", "ASVs", "OTUs"),
    all.x = TRUE
  ) %>%
    relocate(Fasta, .after = ifelse(mode == "ASV", "ASVs", "OTUs")) %>%
  ...
  filtered_data_with_flags <<- raw_data_with_seqs %>%
    # Replace underscores with spaces in both Species and Genus columns
    mutate(
      Species = str_replace_all(Species, "_", " "),
      Genus   = str_replace_all(Genus, "_", " ")
    ) %>%
    # Remove any genus-derived substring from Species
  ...
  if (nrow(validated_data) > 0) {
    ...
    validated_data_updated <- validated_data %>%
      ...
      mutate(
        Validation = ifelse(is.na(NCBI_species), "Validated - DB", Validation),
        Species = ifelse(Validation == "Validated - DB", "", Species),
        Validation = ifelse(Validation == "Validated - DB", "Validated", Validation)
      ) %>%
      select(-Full_Name, -NCBI_species)
    
    # Update main dataset with validation results
    filtered_data_with_flags <<- filtered_data_with_flags %>%
      rows_update(validated_data_updated, by = ifelse(mode == "ASV", "ASVs", "OTUs"))
    ...
    message(paste0("File saved as: ", output_xlsx))
  }
