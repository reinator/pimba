...
#bin/sh
##usage: ./pimba_run.sh -i <input_reads> -o <output_dir> -s <otu_similarity> -a <assign_similarity> -c <coverage> -l <otu_length> -h <hits_per_subject> -g <marker_gene> -t <num_threads> -e <E-value> -d <databases.txt>
...
#USAGE: /bio/pimba_metabarcoding/pimba_run.sh -i AllSamplesCOI_chip1234_good.fasta -o AllSamplesCOI_98clust90assign -s 0.98 -a 0.9 -c 0.9 -l 130 -h 1 -g COI-ALL -t 24 -e 0.1 -d databases.txt
...
SIMILARITY_INT=$(bc -l <<<"${SIMILARITY_ASSIGN}*100")
...
while getopts "i:o:s:a:c:l:h:g:t:e:d:" opt; do
	case $opt in
		i) RAWDATA="$OPTARG"
		;;
		o) OUTPUT="$OPTARG"
...
docker run -id -v $COMMON_PATH:/common/ -v $CURRENT_PATH:/output/ --name vsearch itvdsbioinfo/pimba_vsearch:v2.15.2

echo "Running the VSEARCH Container - --derep_fulllength: "
docker exec -i vsearch /bin/bash -c 'cd /output/'$OUTPUT'; \
...
docker exec -i vsearch  /bin/bash -c 'cd /output/'$OUTPUT'; \
	...
	--id '$SIMILARITY'; chmod 777 '${newfile}_otus1.fasta';'
...
docker run -id -v $COMMON_PATH:/common/ -v $CURRENT_PATH:/output/ --name perl itvdsbioinfo/pimba_perl:v5

echo "Running the Perl Container: "
docker exec -i perl /bin/bash -c 'cd /output/'$OUTPUT'; \
...
docker exec -i vsearch  /bin/bash -c 'cd /output/'$OUTPUT'; \
	...
	--id '$SIMILARITY' --uc '${newfile}'_map.uc; \
...
docker exec -i qiimepipe /bin/bash -c 'cd /output/'$OUTPUT'; \
...
##########################################
#Run LULU to generate a curated otu_table#
...
docker run -id -v $COMMON_PATH:/common/ -v $CURRENT_PATH:/output/ --name blast itvdsbioinfo/pimba_blast:latest

echo "Running the BLAST Container - makeblastdb: "
docker exec -i blast /bin/bash -c 'cd /output/'$OUTPUT'; mkdir lulu_output; cd lulu_output/; \
...
docker exec -i blast /bin/bash -c 'cd /output/'$OUTPUT'/lulu_output/; \
	blastn -db '${newfile}'_otus.fasta -outfmt "6 qseqid sseqid pident" -out match_list.txt \
	-qcov_hsp_perc 80 -perc_identity 84 -query ../'${newfile}'_otus.fasta; \
...
docker run -id -v $COMMON_PATH:/common/ -v $CURRENT_PATH:/output/ --name rdocker itvdsbioinfo/pimba_r:latest bash

echo "Running the R Container - lulu: "
docker exec -i rdocker /bin/bash -c 'cd /output/'$OUTPUT'/lulu_output/; \
...
then
	...
	docker run -id -v $COMMON_PATH:/common/ -v $CURRENT_PATH:/output/ -v $SILVA_DB_16S:/database/ --name qiime itvdsbioinfo/pimba_qiime:latest

	#Assign taxonomy to OTUS using blast method on QIIME
	echo "Running the Qiime Container - assign_taxonomy.py: "
	docker exec -i qiime /bin/bash -c 'cd /output/'$OUTPUT'; \
	...
	docker exec -i qiimepipe /bin/bash -c 'cd /output/'$OUTPUT'/diversity_by_sample; \
...
then
	...
	echo "Creating a Qiime Container: "
	docker run -id -v $COMMON_PATH:/common/ -v $CURRENT_PATH:/output/ -v $GG_DB_16S:/database/ --name qiime itvdsbioinfo/pimba_qiime:latest
...
then
	...
	echo "Creating a Qiime Container: "
	docker run -id -v $COMMON_PATH:/common/ -v $CURRENT_PATH:/output/ -v $RDP_DB_16S:/database/ --name qiime itvdsbioinfo/pimba_qiime:latest
...
then
	...
	echo "Creating a BLAST Container: "
	docker run -id -v $COMMON_PATH:/common/ -v $CURRENT_PATH:/output/ -v $BLASTDB:/blastdb/ --name blast itvdsbioinfo/pimba_blast:latest

	echo "Running the BLAST Container - blastn: "
	docker exec -i blast /bin/bash -c 'cd /output/'$OUTPUT'; export BLASTDB=/blastdb/; \
	...
	#Convert UC to otu-table.txt <<< BMP SCRIPT>>>
	echo "Creating a QiimePipe Container: "
	docker run -id -v $COMMON_PATH:/common/ -v $CURRENT_PATH:/output/ -v $TAXDUMP:/taxdump/ --name qiimepipe itvdsbioinfo/pimba_qiimepipe:v2
	#Generate individual diversity information for each sample in the data and convert the blast file to otu_tax_assignment file from Qiime
...
then
	...
	docker exec -i qiimepipe /bin/bash -c 'cd /output/'$OUTPUT'/diversity_by_sample_ncbi; \
	...
	echo "Running the BLAST Container - blastn: "
	docker exec -i blast /bin/bash -c 'cd /output/'$OUTPUT'; export BLASTDB=/blastdb/; \
		blastn -query k__Fungi.fasta -task megablast -db /blastdb/nt -perc_identity \
		'$SIMILARITY_INT' -qcov_hsp_perc '$COVERAGE_INT' -max_hsps '$HITS_PER_SUBJECT' \
		-max_target_seqs '$HITS_PER_SUBJECT' -evalue '$EVALUE' -parse_deflines -num_threads \
		'$THREADS' -outfmt "6 qseqid sscinames sseqid staxids stitle pident qcovs evalue" > \
...
then
	...
	echo "Creating a Qiime Container: "
	docker run -id -v $COMMON_PATH:/common/ -v $CURRENT_PATH:/output/ -v $ITS_UNITE_DB:/database/ --name qiime itvdsbioinfo/pimba_qiime:latest
...
then
	...
	echo "Running the BLAST Container - blastn: "
	docker exec -i blast /bin/bash -c 'cd /output/'$OUTPUT'; export BLASTDB=/blastdb/; \
		blastn -query '${newfile}'_otus_filtered.fasta -task megablast -db /blastdb/nt -perc_identity \
		'$SIMILARITY_INT' -qcov_hsp_perc '$COVERAGE_INT' -max_hsps '$HITS_PER_SUBJECT' \
		-max_target_seqs '$HITS_PER_SUBJECT' -evalue '$EVALUE' -parse_deflines -num_threads \
		'$THREADS' -outfmt "6 qseqid sscinames sseqid staxids stitle pident qcovs evalue" > \
...
then
	...
	echo "Creating a BLAST Container: "
	docker run -id -v $COMMON_PATH:/common/ -v $CURRENT_PATH:/output/ -v $COI_BOLD_DB:/coibold/ --name blast itvdsbioinfo/pimba_blast:latest
	...
	echo "Running the BLAST Container - blastn: "
	docker exec -i blast /bin/bash -c 'cd /output/'$OUTPUT'; \
	...
	echo "Creating a QiimePipe Container: "
	docker run -id -v $COMMON_PATH:/common/ -v $CURRENT_PATH:/output/ -v $COI_BOLD_DB:/coibold/ --name qiimepipe itvdsbioinfo/pimba_qiimepipe:v2
	...
	echo "Creating a BLAST Container: "
	docker run -id -v $COMMON_PATH:/common/ -v $CURRENT_PATH:/output/ -v $GENE:/gene/ --name blast itvdsbioinfo/pimba_blast:latest
	...
	#Generate individual diversity information for each sample in the data and convert the blast file to otu_tax_assignment file from Qiime
	echo "Creating a QiimePipe Container: "
	docker run -id -v $COMMON_PATH:/common/ -v $CURRENT_PATH:/output/ -v $GENE:/gene/ --name qiimepipe itvdsbioinfo/pimba_qiimepipe:v2
...
docker run -id -v $COMMON_PATH:/common/ -v $CURRENT_PATH:/output/ --name biomformat itvdsbioinfo/pimba_biom:v2.1.10

echo "Running the Biom-Format Container - convert: "
docker exec -i biomformat /bin/bash -c 'cd /output/'$OUTPUT'; \
